...
// Geocodificador simple para ciudades de Chiapas
...
export default function DashboardPrincipal() {
  const { puntos, setPuntos, limpiarPuntosPorTipo, setCentroMapa, setZoomMapa } = useMapContext();
  ...
  // No cargar reportes automáticamente - mapa inicia limpio
  useEffect(() => {
  ...
  // ========== ACTUALIZAR CONDUCTORES EN EL MAPA ==========
useEffect(() => {
  limpiarPuntosPorTipo('conductor');
  ...
  if (conductores.length > 0) {
    const puntosConductores = conductores.map(conductor => {
      ...
      if (esDesconectado) {
        ...
        color = '#ef4444';  // Rojo para conductor normal
        borderColor = '#fff';
      ...
      return {
        ...
        label: `Conductor ${conductor.user_id?.substring(0, 8)}`,
        popupContent: `
          <div style="font-size:12px;min-width:220px;font-family:system-ui,sans-serif;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:8px;background:${color}15;border-radius:8px;border:1px solid ${color}30;">
              <div style="width:24px;height:24px;border-radius:50%;background:${color};border:2px solid ${borderColor};box-shadow:0 0 8px ${shadowColor};flex-shrink:0;"></div>
              <div>
                <span style="font-weight:700;color:${color};font-size:13px;">${esDesconectado ? 'DESCONECTADO' : esDesviado ? 'DESVIADO' : 'EN RUTA'}</span>
                <br><span style="font-size:10px;color:#64748b;">${conductor.velocidad_kmh?.toFixed(1) || '0'} km/h</span>
              </div>
            </div>
            <div style="margin-bottom:2px;"><b>ID:</b> <span style="font-size:10px;">${conductor.user_id?.substring(0, 14)}...</span></div>
            <div style="margin-bottom:2px;"><b>Ruta:</b> ${conductor.ruta_id}</div>
            <div style="margin-bottom:2px;"><b>Pos:</b> ${conductor.lat.toFixed(5)}, ${conductor.lon.toFixed(5)}</div>
            ${conductor.distancia_desvio_m ? `<div style="margin-bottom:2px;color:#ef4444;"><b>Desvío:</b> ${conductor.distancia_desvio_m.toFixed(0)}m</div>` : ''}
            ${conductor.last_telemetry_at ? `<div style="font-size:10px;color:#64748b;margin-top:4px;">Actualizado: ${timeAgo(conductor.last_telemetry_at)}</div>` : ''}
          </div>
        `
      };
    });

    setPuntos(prev => [...prev.filter(p => p.tipo !== 'conductor'), ...puntosConductores]);
  }
}, [conductores, limpiarPuntosPorTipo, setPuntos]);
  // ========== CARGAR REPORTES (solo cuando se busca) ==========
  ...
  try {
    ...
    if (queryBusqueda && queryBusqueda.trim().length >= 2) {
      console.log('🔍 Buscando:', queryBusqueda, '| Tipo:', tipoFiltro || 'todos');
      ...
      try {
        ...
        console.log(`✅ NLP: ${resultadosNLP.length} resultados`);
        
        // Aplicar filtro de tipo SOBRE los resultados del NLP
        if (tipoFiltro) {
          resultadosNLP = resultadosNLP.filter((r: any) => r.tipo === tipoFiltro);
          console.log(`🔍 Después del filtro "${tipoFiltro}": ${resultadosNLP.length}`);
        }
        
        if (resultadosNLP.length > 0) {
          // Cargar API para coordenadas
          ...
          reportes = resultadosNLP
            ...
            .map((r: any) => {
              // ... mapeo existente para obtener coordenadas ...
      ...
      if (reportes.length > 0) {
            ...
            return {
              ...
              popupContent: `
                <div style="font-size:12px;min-width:200px;">
                  ...
                  <span style="font-size:10px;color:#64748b;margin-left:6px;">Relevancia: ${scorePercent}%</span>
                  <p style="margin:6px 0;font-size:11px;">${(reporte.nota_voz || reporte.texto || 'Sin descripción').substring(0, 120)}</p>
        ...
        // Reemplazar solo reportes, mantener conductores y predicciones
        ...
        // Si buscó pero no hay resultados, asegurar que no queden reportes
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 bg-[#0d1b33] border-b border-[#2a4070]/30 px-4 py-2 flex items-center gap-2 overflow-x-auto">
        ...
        <div className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs ${
          conectado ? 'bg-[#10b981]/10 text-[#10b981]' : 'bg-[#ef4444]/10 text-[#ef4444]'
        }`}>
          <span className={`w-2 h-2 rounded-full ${conectado ? 'bg-[#10b981] animate-pulse' : 'bg-[#ef4444]'}`}></span>
          {conectado ? 'WS' : 'WS off'}
        </div>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        {panelActivo && (
          <div className="w-80 flex-shrink-0 bg-[#0d1b33] border-r border-[#2a4070]/30 overflow-y-auto z-20">
            ...
            {panelActivo === 'conductores' && (
              <PanelConductores conductores={conductores} onCentrar={(uid) => { const c = centrarEnConductor(uid); if (c) { setCentroMapa([c.lat, c.lon]); setZoomMapa(14); } }} onEnviarAlerta={(uid, lat, lon) => { const m = prompt('Mensaje:'); if (m) enviarAlertaCritica(uid, m, 'CRITICO', lat, lon).then(() => alert('✅')).catch(() => alert('❌')); }} onCerrar={() => setPanelActivo(null)} />
            )}
          </div>
        )}
        ...
        <div className="flex-1 relative min-w-0">
          {!mapReady && (
            <div className="absolute inset-0 z-10 bg-[#0a1628] flex items-center justify-center">
              <LoadingSpinner message="Cargando mapa..." />
            </div>
          )}
          
          <MapContainer center={[16.5, -93.3]} zoom={9} scrollWheelZoom={true} className="h-full w-full" style={{ background: '#0a1628' }}>
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution='&copy; OpenStreetMap' />
            
            {conductores.map(c => c.polilinea && c.polilinea.length > 1 && (
              <Polyline key={`ruta-${c.user_id}`} positions={c.polilinea.map(p => [p.lat, p.lon] as [number, number])} pathOptions={{ color: c.desviado ? '#ef4444' : '#0ea5e9', weight: 2, opacity: 0.6, dashArray: c.desviado ? '10 5' : undefined }} />
            ))}
  ...
  <CircleMarker
    ...
    // Efecto de pulso para conductores activos
  ...
  >
    <Popup>
      ...
      {/* Leyenda */}
      <div className="flex-shrink-0 bg-[#0d1b33] border-t border-[#2a4070]/30 px-4 py-1.5 flex flex-wrap gap-3 text-xs text-[#94a3b8]">
        <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#ef4444]"></span> Critico</span>
    ...
    <span className="relative flex items-center justify-center">
      <span className="w-3 h-3 rounded-full bg-[#ef4444] border-2 border-white shadow-lg shadow-red-500/50"></span>
    </span>
    Conductor
  </span>
  <span className="flex items-center gap-1.5">
    ...
    Desconectado
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#ef4444] border-2 border-white"></span> Desviado</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#6b7280] border-2 border-white" style={{borderStyle:'dashed'}}></span> Desconectado</span>
      </div>
    </div>
  );
}